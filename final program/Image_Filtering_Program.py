"""
    ...
    File: Image_Filtering_Program.py
    Date: 11/17/17
...
# Check filename selection
print ""
imgname_entry_valid = False
while not imgname_entry_valid:
    imgname = raw_input("Filename of image? ") # Ask for filename

    if os.path.isfile(imgname) == True: # Check if valid file
...
# Define blank method to do nothing
def nothing(x):
    pass
...
# Define method to read color trackbars
def getColorTB():
    # Read in user input from the trackbars
...
# Define method to set colors to images
def setColor():
...
cv2.namedWindow("Greyscale Trackbars") # Create window for greyscale trackbars
...
# Set greyscale trackbars to default values
cv2.setTrackbarPos("g1", "Greyscale Trackbars", 42)
cv2.setTrackbarPos("g2", "Greyscale Trackbars", 85)
cv2.setTrackbarPos("g3", "Greyscale Trackbars", 128)
cv2.setTrackbarPos("g4", "Greyscale Trackbars", 171)
cv2.setTrackbarPos("g5", "Greyscale Trackbars", 214)
...
# Define method to read greyscale trackbar positions and set ranges
def setGreyscale():
...
# Define method to apply masks to color images
def applyMasks():
...
# Define method to create image array
def stackArray():
...
# Initiate while loop
while True:
    # Show image array to window
    cv2.imshow("Images", combArray)

    # Wait 5 ms for keypress
    k = cv2.waitKey(5)

    # Check if the escape or s key was pressed
    if k == 27:
        cv2.destroyAllWindows() # Destroy created windows
        raise SystemExit() # Quit the program
    elif k == ord("s"):
        cv2.imwrite("greyscale_" + imgname, gCImg) # Save greyscale image to file
        cv2.imwrite("multifiltered_" + imgname, fImg) # Save filtered image to file
        cv2.destroyAllWindows() # Destroy created windows
        raise SystemExit() # Quit the program
    else:
        k = cv2.waitKey(1) # Check for keypress again if a key other than escape or s was pressed

    # Run methods to create filtered image again
    getColorTB()
    setColor()
    setGreyscale()
    createMasks()
