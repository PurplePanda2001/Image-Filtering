'''
    ...
    File: Images_07_Different_Colors.py
    Date: 10/31/17
...
while imgname_entry_valid == False:
    ...
    if os.path.isfile("images/" + imgname) == True:
...
colors = ["red", "orange", "yellow", "y_green", "green", "b_green", "g_blue", "l_blue", "blue", "purple", "pink", "magenta"]

# Print available colors
print "Available colors:"
for i in colors:
    print i
print "\nChoose six colors from darkest grey to lightest grey.\n"
...
img_5 [:] = input("Select color 5: ")
img_6 [:] = input("Select color 6: ")
print ""
...
upper_4 = [(GS*4)+3, (GS*4)+3, (GS*4)+3]
lower_5 = [(GS*4)+4, (GS*4)+4, (GS*4)+4]
upper_5 = [(GS*5)+4, (GS*5)+4, (GS*5)+4]
lower_6 = [(GS*5)+5, (GS*5)+5, (GS*5)+5]
upper_6 = [255, 255, 255]
...
fImg_part3 = cv2.bitwise_or(filtered_5, filtered_6, mask = None)
fImg_part1_2 = cv2.bitwise_or(fImg_part1, fImg_part2, mask = None)

fImg = cv2.bitwise_or(fImg_part1_2, fImg_part3, mask = None)
...
# Show avaliable keys to press
print "Press ESC to quit without saving.\nPress S to save and quit.\nPress R to restart program without saving.\n"
...
while True:
    if (k == 27):
        ...
        cv2.destroyAllWindows()
        sys.stdout.flush()
        os.execl("E:\Python Scripts\Engineering 2\Unit 3\Images_07_Different_Colors.py", "Images_07_Different_Colors.py")
    else:
        k = cv2.waitKey(0)
